...
- **smallModel：** 无工具预热消息的回退模型（例如 Claude Code 的探测请求）；默认是 `gpt-5-mini`。网关会对无工具的预热或探测请求强制使用该小模型，以避免消耗 premium 请求。该行为对所有 GitHub Copilot 账户生效，与 `token_based_billing` 无关：token-based 计费账户虽然没有需要节省的 premium 请求配额，但预热请求仍按 token 计费，因此小模型回退依然能降低成本。被 `claudeAutoModel` 命中的请求除外，会保留该模型。
